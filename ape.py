...
with st.sidebar:
    st.header("Controls")
    uploaded_file = st.file_uploader("Upload your PDF here:", type=['pdf'], key="unique_file_uploader")
    summarize_button = st.button("Summarize Each Page")
...
if uploaded_file is not None:
    ...
    if summarize_button:
        ...
        st.header("Summaries")
        for page, summary in sorted(summaries.items()):
            with st.expander(f"Summary for Page {page}"):
                st.write(summary)
